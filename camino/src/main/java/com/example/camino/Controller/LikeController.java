...
public class LikeController {
    ...
    @PostMapping("/add")
    public ResponseEntity addLike(@RequestBody @Valid LikePost likePost){
        likeService.add(likePost);
        return ResponseEntity.status(HttpStatus.OK).body("Like adde");
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateLike(@RequestBody @Valid LikePost likePost, @PathVariable Integer id){
