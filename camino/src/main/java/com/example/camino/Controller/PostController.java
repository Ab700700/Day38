...
public class PostController {
    ...
    @PostMapping("/add/{uid}")
    public ResponseEntity addPost(@PathVariable Integer uid, @RequestBody @Valid Post post){
        return ResponseEntity.status(HttpStatus.OK).body(postService.addPost(uid,post));
    }

    @PutMapping("/update/{id}/{uid}")
    public ResponseEntity updatePost(@PathVariable Integer id , @PathVariable Integer uid,@RequestBody @Valid Post post ){
